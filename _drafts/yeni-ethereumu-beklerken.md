...
Şu an yaşanan en büyük sorun, Ethereum sistemi üzerinde değil, bu sistemi kullanacak onaylayıcıların kullandığı işlemci program (ingilizcesi 'client') konusunda. Merkeziyetsizlik gereği, onaylayıcıların tek bir işlemci program yerine 5–6 farklı program kullanmasını isteyen Ethereum ekibi şu anda bu işlemcilerin birkaçında ortaya çıkan problemleri ve bunların birbirleri ile uyumlu olması sorununu çözmeye çalışıyor. Onun dışında şu an 'bilinen' büyük bir problem yok. Hiç beklenmedik bir problem çıkar mı, onu hep birlikte göreceğiz.

#### Planlarda nasıl bir değişiklik oldu?
Bu arada, başlangıçta Ethereum 2.0 olarak adlandırılan bu değişim sürecinin ismi de değişmiş durumda.  Kısacası, şu anki sistem (eth1) kalıyor ve PoS mekanizması kullanan bir sisteme (eth2) dönüşüyor. Ethereum dünyası buna Ethereum 2.0 yerine birleşme (ingilizcede 'The Merge') [adını veriyor](https://blog.ethereum.org/2022/01/24/the-great-eth2-renaming/). Şu an PoS mekanizmalı eth2 üzerinden işlem geçmeden düzgün bir şekilde çalışıyor. Kısa zaman içinde üzerinden işlem geçerek yapılacak testler de başlıyor.
